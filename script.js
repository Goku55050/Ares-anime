...
function renderWatchPage() {
  ...
  // Next episode button
  setupNextEpisode(animeId, seasonId, episodeId);
...
// NEXT EPISODE FEATURE (uses ordered episode.number)
function setupNextEpisode(animeId, seasonId, episodeId) {
  const anime = animeData[animeId];
  if (!anime || !anime.seasons || !anime.seasons[seasonId]) return;
  const epsEntries = Object.entries(anime.seasons[seasonId].episodes || {});
  if (!epsEntries.length) return;

  // Sort by numeric "number" field to get correct order
  epsEntries.sort((a, b) => {
    const na = (a[1] && a[1].number) || 0;
    const nb = (b[1] && b[1].number) || 0;
    return na - nb;
  });

  const currentIndex = epsEntries.findIndex(([id]) => id === episodeId);
  if (currentIndex === -1 || currentIndex === epsEntries.length - 1) return;

  const nextId = epsEntries[currentIndex + 1][0];
  const container = document.querySelector(".watch-player");
  if (!container) return;

  // Avoid duplicate button
  if (document.getElementById("next-episode")) return;

  const btn = document.createElement("a");
  btn.id = "next-episode";
  btn.className = "btn-primary";
  btn.textContent = "Next Episode ▶";
  btn.href = `watch.html?anime=${animeId}&season=${seasonId}&episode=${nextId}`;
